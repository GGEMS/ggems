...
__host__ __device__ i32 GetHexIndex(f64xyz position, Scene geometry, ui32 adr_geom, f64xyz center, f64xyz u, f64xyz v, f64xyz w)
{
  ...
  //  printf("col %d not round %f \n", col, (((f32)geometry.data_objects[adr_geom+ADR_COLLI_CUBARRAY_VECY] * (((i32)geometry.data_objects[adr_geom+ADR_COLLI_CUBARRAY_NY]
    //                                           - 1 ) / 2.0)) - (f32)position.y) / (f32)geometry.data_objects[adr_geom+ADR_COLLI_CUBARRAY_VECY]);
   ...
   // printf("col %d not round %f / ligne %d not round %f \n", col, (((f32)geometry.data_objects[adr_geom+ADR_COLLI_CUBARRAY_VECY] * (((i32)geometry.data_objects[adr_geom+ADR_COLLI_CUBARRAY_NY]
     //                                          - 1 ) / 2.0)) - (f32)position.y) / (f32)geometry.data_objects[adr_geom+ADR_COLLI_CUBARRAY_VECY], raw, ((f32)geometry.data_objects[adr_geom+ADR_COLLI_LINEAR_VECZ] * ((i32)geometry.data_objects[adr_geom+ADR_COLLI_CUBARRAY_NZ]
       //                                         - 1.0) - (f32)position.z) / (f32)geometry.data_objects[adr_geom+ADR_COLLI_LINEAR_VECZ]);
...
__host__ __device__ f64 GetDistanceHex(f64xyz position, f64xyz direction, f64 radius, f64 cy, f64 cz)
{
    ...
    for (i32 s=0; s<6; s++) {
        ...
        if(dist_plane >= 0.0f) {
            f64xyz pos_temp = fxyz_add(position, fxyz_scale(direction, dist_plane + EPSILON3));
...
__host__ __device__ void get_next_geometry_boundary(Scene geometry, ui32 cur_geom,
    ...
    // Special case of the collimator
          ...
          /*if (distance == 0.0) {
              ...
              f64 temp_distance2 = distance + 0.05;
          
              f64xyz next_pos2 = fxyz_add(pos, fxyz_scale(dir, temp_distance2));  
              ...
              i32 hex = GetHexIndex(next_pos2, geometry, adr_geom, colli_center, u, v, w);
              ...
              if (hex >= 0) {
                  ...
                  f64 dify = fabs(pos.y - (f64)geometry.data_objects[ind_y+hex]);
                  f64 difz = fabs(pos.z - (f64)geometry.data_objects[ind_z+hex]);

                  f64 horiz = (f64)geometry.data_objects[adr_geom+ADR_COLLI_HOLE_RADIUS];
                  f64 verti = ((f64)geometry.data_objects[adr_geom+ADR_COLLI_HOLE_RADIUS] * (2.0/sqrt(3.0))) / 2.0;
                  
                  if(difz >= 2*verti || dify >= horiz || (2*verti*horiz - verti*dify - horiz*difz) <= 0.0 )
                    printf("false \n");

                  printf("dify %f difz %f horiz %f verti %f \n", dify, difz, horiz, verti);
    ...
    while (offset_node < geometry.size_of_nodes[cur_geom]) {
        ...
        if (obj_type == VOXELIZED) {
            ...
            // Volume bounding box
            distance = get_distance_to_object(geometry, adr_child_geom, OBB, pos, dir);
        }
        else {
            // Any other volumes
