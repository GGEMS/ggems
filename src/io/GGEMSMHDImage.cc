...
void GGEMSMHDImage::Read(std::string const& image_mhd_header_filename, std::weak_ptr<cl::Buffer> solid_data_cl)
{
  ...
  for (GGuint i = 0; i < 3; ++i) {
    solid_data_device->position_xyz_.s[i] = solid_data_device->number_of_voxels_xyz_.s[i] * solid_data_device->voxel_sizes_xyz_.s[i] * 0.5f;

    solid_data_device->obb_geometry_.border_min_xyz_.s[i] = -solid_data_device->position_xyz_.s[i];
    solid_data_device->obb_geometry_.border_max_xyz_.s[i] = solid_data_device->position_xyz_.s[i];
  }
