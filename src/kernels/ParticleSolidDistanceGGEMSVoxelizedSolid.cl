...
/*!
  \fn __kernel void particle_solid_distance_ggems_voxelized_solid(GGlong const particle_id_limit, __global GGEMSPrimaryParticles* primary_particle, __global GGEMSVoxelizedSolidData* voxelized_solid_data)
...
)
{
  ...
  GGfloat3 position;
  position.x = primary_particle->px_[global_id];
  position.y = primary_particle->py_[global_id];
  position.z = primary_particle->pz_[global_id];

  // Direction of particle
  GGfloat3 direction;
  direction.x = primary_particle->dx_[global_id];
  direction.y = primary_particle->dy_[global_id];
  direction.z = primary_particle->dz_[global_id];
