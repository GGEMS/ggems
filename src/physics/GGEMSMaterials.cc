...
GGEMSChemicalElement::GGEMSChemicalElement(void)
{
  GGcout("GGEMSChemicalElement", "GGEMSChemicalElement", 3)
    << "Allocation of GGEMS chemical element..." << GGendl;
}
...
{
  GGcout("GGEMSChemicalElement", "~GGEMSChemicalElement", 3)
    << "Deallocation of GGEMS chemical element..." << GGendl;
}
...
void GGEMSMaterialsDatabase::LoadMaterialsDatabase(std::string const& filename)
{
  ...
  while (std::getline(database_stream, line)) {
    ...
    material.name_ = GGEMSMaterialReader::ReadMaterialName(line);
    ...
    materials_.insert(std::make_pair(material.name_, material));
  }
...
void GGEMSMaterialsDatabase::LoadChemicalElements(void)
{
  GGcout("GGEMSMaterialsDatabase", "LoadChemicalElements", 0)
...
void GGEMSMaterialsDatabase::AddChemicalElements(
  std::string const& element_name, GGushort const& element_Z,
  GGdouble const& element_A, GGdouble const& element_I)
{
  GGcout("GGEMSMaterialsDatabase", "AddChemicalElements", 3)
    << "Adding element: **** " << element_name << " **** in GGEMS..." << GGendl;
