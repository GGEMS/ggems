...
#endif
{
  ...
  #else
  GGfloat energy_bins_[GGEMSProcess::MAX_CROSS_SECTION_TABLE_NUMBER_BINS]; /*!< Energy in bin (220 by default) */
  #endif
  ...
  GGfloat photon_cross_sections_[NUMBER_PHOTON_PROCESSES][256*MAX_CROSS_SECTION_TABLE_NUMBER_BINS]; /*!< Photon cross sections */
  GGfloat photon_cross_sections_per_atom_[NUMBER_PHOTON_PROCESSES][101*MAX_CROSS_SECTION_TABLE_NUMBER_BINS]; /*!< Photon cross sections per atom in mm-1, useful for Photoelectric effect and Rayleigh */
  ...
  #else
  GGuchar index_photon_cs_[GGEMSProcess::NUMBER_PHOTON_PROCESSES]; /*!< Index of activated photon process, ex: if only Rayleigh activate index_photon_cs[0] = 2 */
  GGfloat photon_cross_sections_[GGEMSProcess::NUMBER_PHOTON_PROCESSES][256*GGEMSProcess::MAX_CROSS_SECTION_TABLE_NUMBER_BINS]; /*!< Photon cross sections per material in mm-1 */
  GGfloat photon_cross_sections_per_atom_[GGEMSProcess::NUMBER_PHOTON_PROCESSES][101*GGEMSProcess::MAX_CROSS_SECTION_TABLE_NUMBER_BINS]; /*!< Photon cross sections per atom in mm-1, useful for Photoelectric effect and Rayleigh, 100 chemical elements + 1 first empty element */
  GGfloat rayleigh_scatter_factor_[101*GGEMSProcess::MAX_CROSS_SECTION_TABLE_NUMBER_BINS]; /*!< For Rayleigh scattering a scatter factor by chemical element is necessary */
  #endif
