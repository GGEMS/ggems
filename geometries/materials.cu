...
void MaterialManager::m_copy_materials_table_cpu2gpu() {
    ...
    HANDLE_ERROR( cudaMalloc((void**) &mat_table_d.photon_energy_cut, n*sizeof(f32)) );
    HANDLE_ERROR( cudaMalloc((void**) &mat_table_d.electron_energy_cut, n*sizeof(f32)) );
    ...
    HANDLE_ERROR( cudaMemcpy(mat_table_d.photon_energy_cut, mat_table_h.photon_energy_cut,
                             n*sizeof(f32), cudaMemcpyHostToDevice) );
    HANDLE_ERROR( cudaMemcpy(mat_table_d.electron_energy_cut, mat_table_h.electron_energy_cut,
                             n*sizeof(f32), cudaMemcpyHostToDevice) );
...
void MaterialManager::m_build_materials_table(GlobalSimulationParameters params) {
    ...
    mat_table_h.nb_materials = m_materials_list.size();
    mat_table_h.nb_elements = (ui16*)malloc(sizeof(ui16)*mat_table_h.nb_materials);
    mat_table_h.index = (ui16*)malloc(sizeof(ui16)*mat_table_h.nb_materials);
    mat_table_h.nb_atoms_per_vol = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.nb_electrons_per_vol = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.electron_mean_excitation_energy = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.rad_length = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.photon_energy_cut = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.electron_energy_cut = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fX0 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fX1 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fD0 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fC = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fA = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fM = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.density = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fF1 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fF2 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fEnergy0 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fEnergy1 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fEnergy2 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fLogEnergy1 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fLogEnergy2 = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    mat_table_h.fLogMeanExcitationEnergy = (f32*)malloc(sizeof(f32)*mat_table_h.nb_materials);
    ...
    i=0; while (i < mat_table_h.nb_materials) {
        // get mat name
        ...
        // get nb of elements
        mat_table_h.nb_elements[i] = cur_mat.nb_elements;

        // compute index
        mat_table_h.index[i] = access_index;
    ...
    mat_table_h.nb_elements_total = access_index;
    mat_table_h.mixture = (ui16*)malloc(sizeof(ui16)*access_index);
    mat_table_h.atom_num_dens = (f32*)malloc(sizeof(f32)*access_index);
    ...
    i=0; while (i < mat_table_h.nb_materials) {
        ...
        // get density
        mat_table_h.density[i] = cur_mat.density / gram;

        // G4 material
        G4Material *g4mat = new G4Material("tmp", cur_mat.density, cur_mat.nb_elements);

        mat_table_h.nb_atoms_per_vol[i] = 0.0f;
        mat_table_h.nb_electrons_per_vol[i] = 0.0f;
        ...
        j=0; while (j < cur_mat.nb_elements) {
            ...
            // store Z
            mat_table_h.mixture[fill_index] = m_material_db.elements_Z[elt_name];

            // compute atom num dens (Avo*fraction*dens) / Az
            mat_table_h.atom_num_dens[fill_index] = Avogadro/m_material_db.elements_A[elt_name] *
                                                        cur_mat.mixture_f[j]*cur_mat.density;

            // compute nb atoms per volume
            mat_table_h.nb_atoms_per_vol[i] += mat_table_h.atom_num_dens[fill_index];

            // compute nb electrons per volume
            mat_table_h.nb_electrons_per_vol[i] += mat_table_h.atom_num_dens[fill_index] *
        ...
        // electron data
        mat_table_h.electron_mean_excitation_energy[i] = g4mat->GetIonisation()->GetMeanExcitationEnergy();
        mat_table_h.rad_length[i] = g4mat->GetRadlen();

        // cut (in energy for now, but should be change for range cut) TODO
        mat_table_h.photon_energy_cut[i] = params.photon_cut;
        mat_table_h.electron_energy_cut[i] = params.electron_cut;

        // eIonisation correction
        mat_table_h.fX0[i] = g4mat->GetIonisation()->GetX0density();
        mat_table_h.fX1[i] = g4mat->GetIonisation()->GetX1density();
        mat_table_h.fD0[i] = g4mat->GetIonisation()->GetD0density();
        mat_table_h.fC[i] = g4mat->GetIonisation()->GetCdensity();
        mat_table_h.fA[i] = g4mat->GetIonisation()->GetAdensity();
        mat_table_h.fM[i] = g4mat->GetIonisation()->GetMdensity();

        //eFluctuation parameters
        mat_table_h.fF1[i] = g4mat->GetIonisation()->GetF1fluct();
        mat_table_h.fF2[i] = g4mat->GetIonisation()->GetF2fluct();
        mat_table_h.fEnergy0[i] = g4mat->GetIonisation()->GetEnergy0fluct();
        mat_table_h.fEnergy1[i] = g4mat->GetIonisation()->GetEnergy1fluct();
        mat_table_h.fEnergy2[i] = g4mat->GetIonisation()->GetEnergy2fluct();
        mat_table_h.fLogEnergy1[i] = g4mat->GetIonisation()->GetLogEnergy1fluct();
        mat_table_h.fLogEnergy2[i] = g4mat->GetIonisation()->GetLogEnergy2fluct();
        mat_table_h.fLogMeanExcitationEnergy[i] = g4mat->GetIonisation()->GetLogMeanExcEnergy();
        ++i;
    }
