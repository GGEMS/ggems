...
f32 ElectronBremmsstrahlung_CSPA( f32 Z, f32 cut, f32 Ekine )
{
...
f32 ElectronBremmsstrahlung_CS( MaterialsTable materials, f32 Ekine, f32 max_E, ui8 mat_id )
{
    i32 i, n, nn, nmax = 100;
    f32 Cross = 0.;
    f32 kmax, kmin, vmin, vmax, totalEnergy, kp2;
    f32 u, fac, c, v, dv, y;
    f32 tmax = std::min ( max_E, Ekine );
    f32 cut = std::max ( materials.photon_energy_cut[mat_id], 0.0001f ); // 0.1 keV
    ...
    vmin = log ( kmin );
    vmax = log ( kmax );
    nn= ( i32 ) ( nmax* ( vmax-vmin ) / ( log ( highKinEnergy )-vmin ) );
