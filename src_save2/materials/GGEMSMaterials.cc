...
GGEMSMaterials::GGEMSMaterials(void)
{
  ...
  // Allocating a buffer for each device
  material_tables_ = new cl::Buffer*[opencl_manager.GetNumberOfActivatedDevice()];
...
GGEMSMaterials::~GGEMSMaterials(void)
{
  ...
  for (GGsize i = 0; i < opencl_manager.GetNumberOfActivatedDevice(); ++i) {
    opencl_manager.Deallocate(material_tables_[i], sizeof(GGEMSMaterialTables));
  }
...
void GGEMSMaterials::PrintInfos(void) const
{
  ...
  for (GGsize i = 0; i < opencl_manager.GetNumberOfActivatedDevice(); ++i) {
    ...
    GGEMSMaterialTables* material_table_device = opencl_manager.GetDeviceBuffer<GGEMSMaterialTables>(material_tables_[i], sizeof(GGEMSMaterialTables), device_index);

    // Getting list of activated materials
    GGcout("GGEMSMaterials", "PrintInfos", 0) << GGendl;
    GGcout("GGEMSMaterials", "PrintInfos", 0) << "Device: " << opencl_manager.GetDeviceName(device_index) << GGendl;
    ...
    opencl_manager.ReleaseDeviceBuffer(material_tables_[i], material_table_device, device_index);
  }
}
...
void GGEMSMaterials::BuildMaterialTables(void)
{
  ...
  for (GGsize d = 0; d < opencl_manager.GetNumberOfActivatedDevice(); ++d) {
    // Get the index of activated device
    GGsize device_index = opencl_manager.GetIndexOfActivatedDevice(d);

    // Allocating memory for material tables in OpenCL device
    material_tables_[d] = opencl_manager.Allocate(device_index, nullptr, sizeof(GGEMSMaterialTables), CL_MEM_READ_WRITE, "GGEMSMaterials");

    // Getting the OpenCL pointer on material tables
    GGEMSMaterialTables* material_table_device = opencl_manager.GetDeviceBuffer<GGEMSMaterialTables>(material_tables_[d], sizeof(GGEMSMaterialTables), device_index);
    ...
    // Release the pointer, mandatory step!!!
    opencl_manager.ReleaseDeviceBuffer(material_tables_[d], material_table_device, device_index);
  }
...
GGfloat GGEMSMaterials::GetDensity(std::string const& material_name) const
{
  ...
  // Get the index of first activated device
  GGsize device_index = opencl_manager.GetIndexOfActivatedDevice(0);

  // Getting the OpenCL pointer on material tables
  GGEMSMaterialTables* material_table_device = opencl_manager.GetDeviceBuffer<GGEMSMaterialTables>(material_tables_[0], sizeof(GGEMSMaterialTables), device_index);
  ...
  opencl_manager.ReleaseDeviceBuffer(material_tables_[0], material_table_device, device_index);
