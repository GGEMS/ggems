...
__host__ __device__ void photon_navigator_simple(ParticleStack &particles, ui32 part_id,
    ...
    f32 interaction_distance, cross_section;
    f32 xmin, xmax, ymin, ymax, zmin, zmax;
    f32xyz u, v, w;
    f32xyz pos_obb, obb_center, pos_aabb;
  ...
  //  printf(" obj type %d \n", obj_type);
    ...
    if (obj_type == VOXELIZED) {     
         if (!test_point_AABB(pos, (f32)geometry.data_objects[adr_geom+ADR_AABB_XMIN], (f32)geometry.data_objects[adr_geom+ADR_AABB_XMAX],
   ...
   // printf("move particle %f %f %f dir %f %f %f \n", pos.x, pos.y, pos.z, dir.x, dir.y, dir.z);
    ...
    xmin = (f32)geometry.data_objects[ADR_AABB_XMIN]; // adr_world_geom = 0
    xmax = (f32)geometry.data_objects[ADR_AABB_XMAX];
    ymin = (f32)geometry.data_objects[ADR_AABB_YMIN];
    ymax = (f32)geometry.data_objects[ADR_AABB_YMAX];
    zmin = (f32)geometry.data_objects[ADR_AABB_ZMIN];
    zmax = (f32)geometry.data_objects[ADR_AABB_ZMAX];
    ...
    if (parameters.digitizer_flag &&
        ...
        pos_obb = fxyz_sub(pos, obb_center);
...
__host__ __device__ void photon_navigator_double(ParticleStack &particles, ui32 part_id,
    ...
    f64xyz dir, obb_center, pos_obb, pos_aabb;
    f64 next_interaction_distance = F64_MAX;
    f64 interaction_distance, cross_section;
   ...
   // printf("move particle %f %f %f dir %f %f %f \n", pos.x, pos.y, pos.z, dir.x, dir.y, dir.z);
    ...
    // Check world boundary
    xmin = (f64)geometry.data_objects[ADR_AABB_XMIN]; // adr_world_geom = 0
    xmax = (f64)geometry.data_objects[ADR_AABB_XMAX];
    ymin = (f64)geometry.data_objects[ADR_AABB_YMIN];
    ymax = (f64)geometry.data_objects[ADR_AABB_YMAX];
    zmin = (f64)geometry.data_objects[ADR_AABB_ZMIN];
    zmax = (f64)geometry.data_objects[ADR_AABB_ZMAX];
...
__host__ __device__ void photon_navigator_raytracing_colli(ParticleStack &particles, ui32 part_id,
    ...
    f64xyz pos, dir;
    f64 next_interaction_distance = F64_MAX;
    f64 interaction_distance, cross_section, hole_radius, distance;
    f64 xmin, xmax, ymin, ymax, zmin, zmax;
    f64 aabb_xmin, aabb_xmax, aabb_ymin, aabb_ymax, aabb_zmin, aabb_zmax;
    f64xyz u, v, w;
    f64xyz pos_obb, pos_aabb, obb_center, colli_center;
    f64 half_colli_size_x, half_colli_size_y, half_colli_size_z;
    f64xyz ray_obb, p, d, temp;
    f64xyz pos_temp, pos_test;
    ...
    if (obj_type == COLLI) {
        ...
        aabb_xmin = (f64)geometry.data_objects[adr_geom+ADR_AABB_XMIN];
        aabb_xmax = (f64)geometry.data_objects[adr_geom+ADR_AABB_XMAX];
        aabb_ymin = (f64)geometry.data_objects[adr_geom+ADR_AABB_YMIN];
        aabb_ymax = (f64)geometry.data_objects[adr_geom+ADR_AABB_YMAX];
        aabb_zmin = (f64)geometry.data_objects[adr_geom+ADR_AABB_ZMIN];
        aabb_zmax = (f64)geometry.data_objects[adr_geom+ADR_AABB_ZMAX];
        ...
        if (hex < 0) {
            ...
            half_colli_size_x = (aabb_xmax - aabb_xmin) * 0.5;
            half_colli_size_y = (aabb_ymax - aabb_ymin) * 0.5;
            half_colli_size_z = (aabb_zmax - aabb_zmin) * 0.5;
            
            hole_radius = geometry.data_objects[adr_geom+ADR_COLLI_HOLE_RADIUS];
            ...
            ray_obb = fxyz_sub(pos, colli_center);
            ...
            temp.x = p.x;
            temp.y = p.y - geometry.data_objects[ind_y+hex];
            temp.z = p.z - geometry.data_objects[ind_z+hex];
           ...
           // printf("centerofhex y %f z %f \n", geometry.data_objects[adr_geom+ADR_COLLI_CENTEROFHEXAGONS_Y+hex], 
            ...
            // Move the particle
            pos_temp = fxyz_add(pos, fxyz_scale(dir, distance));
            
            i32 hex_test = GetHexIndex(pos_temp, geometry, adr_geom, colli_center, u, v, w);
            
            if (hex_test == hex) {
              
                next_interaction_distance = distance + EPSILON3; // Overshoot
                pos_test = fxyz_add(pos, fxyz_scale(dir, next_interaction_distance));
