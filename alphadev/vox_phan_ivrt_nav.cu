...
__device__ void VPIVRTN::track_to_out_analog( ParticlesData *particles,
                                    ...
                                    ui32xyzw nb_sup_voxel )
{
    ...
    f32 CS_max = ( E_index == 0 )? mumax_table[CS_max_index]:
                linear_interpolation(photon_CS_table->E_bins[E_index-1], mumax_table[CS_max_index-1],
    ...
    f32 sv_vox_xmin = sv_index_phantom.x * sv_spacing_x - vol->off_x;
    f32 sv_vox_ymin = sv_index_phantom.y * sv_spacing_y - vol->off_y;
    f32 sv_vox_zmin = sv_index_phantom.z * sv_spacing_z - vol->off_z;
    ...
    if ( next_discrete_process != GEOMETRY_BOUNDARY )
    {
        ...
        if ( next_discrete_process == PHOTON_COMPTON )
        {
            Compton_SampleSecondaries_standard( particles, materials->electron_energy_cut[mat_id],
                       parameters->secondaries_list[ELECTRON], part_id );
        }

        if ( next_discrete_process == PHOTON_PHOTOELECTRIC )
        {
            Photoelec_SampleSecondaries_standard( particles, materials, photon_CS_table,
...
__global__ void VPIVRTN::kernel_device_track_to_out_analog ( ParticlesData *particles,
                                                   const VoxVolumeData<ui16> *vol,
                                                   const MaterialsData *materials,
                                                   const PhotonCrossSectionData *photon_CS_table,
                                                   const GlobalSimulationParametersData *parameters )
{
    ...
    {
        VPIVRTN::track_to_out_analog( particles, vol, materials, photon_CS_table, parameters, id );
    }
}
                                                   ...
                                                   ui32xyzw nb_sup_voxel )
{
    ...
    while ( particles->status[id] != PARTICLE_DEAD && particles->status[id] != PARTICLE_FREEZE )
    {
        VPIVRTN::track_to_out_svw( particles, vol, materials, photon_CS_table, parameters, id, mumax_table,
                                mumax_index_table, nb_bins_sup_voxel, nb_sup_voxel );
    }
}
...
ui64 VoxPhanIVRTNav::m_get_memory_usage()
{
    ...
    // If Woodcock or Super Voxel Woodcock
...
void VoxPhanIVRTNav::track_to_in(ParticlesData *d_particles )
{
    ...
    VPIVRTN::kernel_device_track_to_in<<<grid, threads>>>( d_particles,
                                                           m_phantom.h_volume->xmin,
                                                           m_phantom.h_volume->xmax,
                                                           m_phantom.h_volume->ymin,
                                                           m_phantom.h_volume->ymax,
                                                           m_phantom.h_volume->zmin,
                                                           m_phantom.h_volume->zmax,
...
void VoxPhanIVRTNav::track_to_out(ParticlesData *d_particles )
{
    ...
    {
        VPIVRTN::kernel_device_track_to_out_analog<<<grid, threads>>> ( d_particles,
                                                                 m_phantom.d_volume,
    ...
    {
        VPIVRTN::kernel_device_track_to_out_woodcock<<<grid, threads>>> ( d_particles,
                                                                          m_phantom.d_volume,
                                                                          m_materials.d_materials,
                                                                          m_cross_sections.d_photon_CS,
                                                                          md_params,
    ...
    {
        VPIVRTN::kernel_device_track_to_out_svw<<<grid, threads>>> ( d_particles,
                                                                     m_phantom.d_volume,
                                                                     m_materials.d_materials,
                                                                     m_cross_sections.d_photon_CS,
                                                                     md_params,
