...
__host__ __device__ void photon_navigator(ParticleStack &particles, ui32 part_id,
   ...
   /* if (part_id == 88 && obj_type == COLLI) {
     ...
     printf("photon_navigator dir %f %f %f cur_id_geom %d obj_type %d energy %f \n", dir.x, dir.y, dir.z,
   ...
   // printf(" obj type %d \n", obj_type);
    ...
    get_next_geometry_boundary(geometry, cur_id_geom, pos, dir, interaction_distance, hit_id_geom);
     ...
     if (cur_id_geom == 4) {
        ...
        FILE *cfile = fopen("Sortie.txt", "a"); 
    
        fprintf(cfile, "part %d cover colli \n", part_id);
    ...
    if (interaction_distance <= next_interaction_distance) {
       ...
       /* if (cur_id_geom == 3) {
        ...
        FILE *cfile = fopen("Count.txt", "a"); 
    
        fprintf(cfile, "part %d GEOM dist_geom %f dist %f\n", part_id, interaction_distance, next_interaction_distance);
    ...
    if (next_discrete_process == PHOTON_COMPTON) {
      ...
      /*  if (cur_id_geom == 3) {
       ...
       FILE *cfile = fopen("Count.txt", "a"); 
        ...
        fprintf(cfile, "part %d COMPTON dist_geom %f dist %f\n", part_id, interaction_distance, next_interaction_distance);
    ...
    if (next_discrete_process == PHOTON_PHOTOELECTRIC) {
        ...
        fprintf(cfile, "part %d PE dist_geom %f dist %f\n", part_id, interaction_distance, next_interaction_distance);
    ...
    if (next_discrete_process == PHOTON_RAYLEIGH) {
     ...
     /*  if (cur_id_geom == 3) {
        ...
        fprintf(cfile, "part %d RAYLEIGH dist_geom %f dist %f\n", part_id, interaction_distance, next_interaction_distance);
    ...
    if (parameters.digitizer_flag &&
        ...
        if (pulses.pu1_nb_hits[part_id] == 0) {
            ...
            FILE *cfile = fopen("Pulses.txt", "a"); 
    
            fprintf(cfile, "pulse %d pos %f %f %f id_geom %d \n", part_id, pulses.pu1_px[part_id], 
                            pulses.pu1_py[part_id], pulses.pu1_pz[part_id], cur_id_geom);
