...
__host__ void cpu_photon_navigator(ParticleStack &particles, unsigned int part_id,
    ...
    unsigned int E_index = binary_search(particles.E[part_id], photon_CS_table.E_CS,
                                         photon_CS_table.nb_bins, id_mat*photon_CS_table.nb_bins);

    // If photoelectric
    if (parameters.physics_list[PHOTON_PHOTOELECTRIC]) {      
        cross_section = linear_interpolation(photon_CS_table.E_CS[E_index-1],
                                             photon_CS_table.PhotoElectric_Std_CS[E_index-1],
                                             photon_CS_table.E_CS[E_index],
